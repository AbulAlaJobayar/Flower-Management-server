...
const getAllProductIntoDB = async (payload: any) => {
    ...
    const [data, totalData] = await Promise.all([
        productQuery
            .filter()
            .search(["name", "price", "bloomDate", "quantity", "color", "category", "size", "fragrance"]) // to make dynamic search use like ['']
            .sort()
            .paginate()
            .fields().modelQuery,
...
const updateProductIntoDB = async (userData: JwtPayload, id: string, payload: Partial<TProduct>) => {
